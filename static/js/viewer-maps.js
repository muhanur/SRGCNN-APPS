...
function getArrows(arrLatlngs, color, arrowCount, mapObj) {
    ...
    for (var i = 1; i < arrLatlngs.length; i++) {
        var icon = L.divIcon({ 
			...
			bgPos: [5, 5], 
			html: '<div style="' + color + ';transform: rotate(' + getAngle(arrLatlngs[i - 1], arrLatlngs[i], 0).toString() + 'deg)">▶</div>'
		});
...
var ResultLayer = L.geoJson(ResultFeature, { pane: 'right' }).addTo(map);
...
let table = new DataTable('#predTable', {
    fixedColumns: {
        left: 2
    },
    scrollX: true,
...
table.clear().rows.add(predictData).draw();
